...
/** Cette classe a pour but de creer la carte du jeu.
 * @author Yassir BOUISSE
 * @version 1
 */
...
class CreationMap {
    ...
    private BufferedImage bowserLeft, bowserRight, koopaLeft, koopaRight, drapeauFin;
	...
	BufferedImage lutin2 = chargeurImage.loadImage("/sprite2.png");
	...
	this.bowserLeft = chargeurImage.getSubImage(lutin2, 2, 4, 48, 48);
	this.bowserRight = chargeurImage.getSubImage(lutin2, 4, 4, 48, 48);
	...
	int bowser = new Color(0, 255, 255).getRGB();
	...
	for (int x = 0; x < imageMap.getWidth(); x++) {
		...
		if (currentPixel == briqueOrdinaire) {
		    ...
		    Ennemi ennemie = new Bowser(xLocation, yLocation, this.bowserLeft);
		    ((Bowser)ennemie).setRightImage(bowserRight);
